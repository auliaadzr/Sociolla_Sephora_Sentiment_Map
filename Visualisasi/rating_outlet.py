...
def main():
    ...
    try:
        ...
        produk_stats = df.groupby(['Outlet', 'e_commerce', 'outlet_id', 'lat', 'lon', 'produk_id', 'Rating_global_produk']).agg(
            Rs=('rating', 'mean'),
            n=('rating', 'count')
        ).reset_index()
        ...
        outlet_final = produk_stats.groupby(['Outlet', 'e_commerce', 'outlet_id', 'lat', 'lon']).agg(
